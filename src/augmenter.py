...
class Augmenter:
    ...
    def __call__(self, img: Union[Image.Image, torch.Tensor], mask: Union[Image.Image, torch.Tensor]):
        ...
        img: Image.Image = self.img_resize_transform(img)
        mask: Image.Image = self.mask_resize_transform(mask)
        ...
        angle = self.random_state.uniform(-10, 10)
        tx = self.random_state.uniform(-0.08, 0.08)
        ty = self.random_state.uniform(-0.08, 0.08)
        scale = self.random_state.uniform(0.8, 1.15)
        shear = self.random_state.uniform(-10, 10)
        ...
        if self.random_state.random() < self.perspective_prob:
            ...
            img = F.perspective(img, start, end, interpolation=F.InterpolationMode.BILINEAR, fill=0)
            mask = F.perspective(mask, start, end, interpolation=F.InterpolationMode.NEAREST, fill=0)
        ...
        if self.random_state.random() < self.brightness_prob:
            ...
            img = F.adjust_brightness(img, self.random_state.uniform(1 - self.brightness_prob, 1 + self.brightness_prob))
        if self.random_state.random() < self.contrast_prob:
            self.logger.debug(f'Applying contrast adjustment augmentation')
            img = F.adjust_contrast(img, self.random_state.uniform(1 - self.contrast_prob, 1 + self.contrast_prob))
        if self.random_state.random() < self.saturation_prob:
            self.logger.debug(f'Applying saturation adjustment augmentation')
            img = F.adjust_saturation(img, self.random_state.uniform(1 - self.saturation_prob, 1 + self.saturation_prob))
        if self.random_state.random() < self.blur_prob:
            self.logger.debug(f'Applying gaussian blur augmentation')
            k = max(3, int(0.01 * min(img.size)) | 1)
        ...
        if self.random_state.random() < self.noise_prob:
            ...
            noise = self.np_random_state.randn(*arr.shape).astype(np.float32) * self.noise_sigma_prob
    ...
    def _object_preserving_crop(self, img: Image.Image, mask: Image.Image, size: Tuple[int, int]):
        th, tw = size
        w, h = img.size
        if w < tw or h < th:
        ...
        img = F.center_crop(img, (th, tw))
        mask = F.center_crop(mask, (th, tw))
        return img, mask
    ...
    def _snap_mask_to_known_values(self, mask: Image.Image) -> Image.Image:
        ...
        d = np.abs(arr16[..., None] - _vals_np[None, None, :])   # [H,W,K]
        idx = d.argmin(axis=2).astype(np.int32)
        snapped = _vals_np[idx].astype(np.uint8)
